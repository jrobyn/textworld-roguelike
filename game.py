...
def fightSystem(Uhr,Uhp,Uat,Uag,Mname,Mhp,Mat,Mag):
    ...
    print("Fight system not yet implemented.")
    print("YOU WIN!")
    bg = (random.randrange(1,5) * 5)
    print("You won {0} gold.".format(bg))
    return bg
...
while rollChar == False:
    ...
    Uhp = 50 + (random.randrange(0,20)) * 5
    Uat = (random.randrange(3,7)) + (random.randrange(0,2))
    Uag = (random.randrange(3,10))
...
while game == True:
    print()
    print("Current stage: {0}\n{1}/{2}hp | {3} gold | {4} keys".format(stage,Uhr,Uhp,gp,keys))
    input("Press enter to pick a card.")
    print("Picking a card...")
    print()
    event = random.randrange(0,13)
    ...
    if event == 4:
        ...
        gp += fightSystem(Uhr,Uhp,Uat,Uag,"Slime Monster",30,2,4)
        undeadLast = False
    if event == 5:
        print("You encounter a Skeleton!\n")
        gp += fightSystem(Uhr,Uhp,Uat,Uag,"Skeleton",30,2,4)
        undeadLast = True
    if event == 6:
        print("You spend the night in a haunted inn.")
        if undeadLast == True:
            print("You encounter a Skeleton!\n")
            gp += gp + fightSystem(Uhr,Uhp,Uat,Uag,"Skeleton",30,2,4)
            undeadLast = True
        else:
    ...
    if event == 8:
        ...
        if gp > 49:
            ...
            print("You don't have enough money to accept her offer.")
    if event == 9:
        print("You end up getting into a brawl with a rowdy villager.")
        skillCheck = random.randrange(4,6)
        print("Skill check... Villager attack: {0}.".format(skillCheck))
        if Uat < skillCheck:
            if gp > 70:
                gp -= 70
                print("The villager knocks you off guard and takes 70 gold pieces.")
            else:
                gp = 0
                Uhr -= 5
                print("The villager knocks you off guard and takes your gold purse.")
                print("You lose 5hp.")
        else:
            gp += 70
